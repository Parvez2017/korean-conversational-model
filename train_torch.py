...
parser = argparse.ArgumentParser(description='Chatbot based on KoGPT-2')
...
class CharDataset(Dataset):
    def __init__(self, chats, tok_path, vocab, max_len=300):
        self._data = chats
    ...
    def __getitem__(self, idx):
        ...
        if q_len + a_len > self.max_len:
            change = q_len + a_len - self.max_len
            if self.max_len - q_len > 0:
                q_len = q_len - change - 1
                q_toked = q_toked[-q_len:]
                assert q_len == len(q_toked)

            elif self.max_len - q_len < 0:
                q_len = q_len - change- 1
                q_toked = q_toked[-q_len:]
